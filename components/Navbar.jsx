  ...
  return (
    <nav className="relative sticky top-0 z-50 flex items-center justify-between px-6 md:px-16 lg:px-32 py-3 border-b border-gray-200 text-gray-700 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/50 shadow-sm">
      ...
      <div className="flex items-center gap-4 lg:gap-8 max-md:hidden">
        ...
        <Link href="/home" className={`group relative flex items-center gap-2 transition hover:text-gray-900 ${pathname === '/home' ? 'text-gray-900' : ''}`}>
        ...
        <SignedIn>
          <UserButton>
            <UserButton.MenuItems>
              <UserButton.Action label="My Orders" labelIcon={<BagIcon />} onClick={() => router.push('/home/my-orders')} />
              <UserButton.Action label="Bookings" labelIcon={<CartIcon />} onClick={() => router.push('/home/my-orders')} />
            </UserButton.MenuItems>
          </UserButton>
        </SignedIn>
        <SignedOut>
      ...
      {mobileOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-200 shadow-sm">
          <div className="px-6 py-3 flex flex-col gap-2">
            ...
            <Link onClick={() => setMobileOpen(false)} href="/home" className="flex items-center gap-2 py-2">
              <Search className="w-4 h-4" />
              Find Services
            </Link>
            <Link onClick={() => setMobileOpen(false)} href="/" className="flex items-center gap-2 py-2">
